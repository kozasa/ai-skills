---
name: human-facing-communication
...
Present agent work so a human can understand and decide quickly. Choose the delivery format from the situation; version 1 uses ordinary prose or `$run-explainer-page` FAST HTML.
...
6. Save it to a local temporary JSON file and invoke `$run-explainer-page` in FAST mode.
...
Pass the local JSON file to `$run-explainer-page --fast`. Its installed `scripts/render_fast.py` owns validation, escaping, templating, output creation, and optional browser opening. Do not reproduce the HTML template in this skill.
